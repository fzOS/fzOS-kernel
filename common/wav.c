...
int play_wav(WavAudioInfo* info_buffer,void* file_buffer,HDAConnector* connector)
{
    ...
    // /2.
    int off = (info_buffer->data_length/2) & ~(0xFFF);
    desc->length = off;
    desc->address = (U64)pcm_buffer&(~KERNEL_ADDR_OFFSET);
    desc->interrupt_on_completion = 1;
    desc[1].length = info_buffer->data_length-off;
    desc[1].address = ((U64)pcm_buffer&(~KERNEL_ADDR_OFFSET))+off;
    desc[1].interrupt_on_completion = 1;
    int stream_no;
