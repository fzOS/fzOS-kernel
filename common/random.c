...
typedef struct {
    DeviceTreeNode node;
    CharDev dev;
} RandomDeviceTreeNode;
static int g_use_software_random = 0;
static U64 g_software_random_next = 1;
U8 random_getchar(CharDev* dev)
{
    (void)dev;
...
int init_random(void)
{
    if(get_processor_rdseed_support()!=FzOS_SUCCESS) {
        ...
        g_use_software_random=1;
        //使用RTC时间作为种子。
        RTCTime time;
        read_rtc(&time);
        g_software_random_next = (time.century*1000+time.day*100+time.month*10+time.hour*20+time.minute*3);
    }
    //在这里初始化一个设备树结点……？
    RandomDeviceTreeNode* random_node = allocate_page(1);
    memset(random_node,0,sizeof(RandomDeviceTreeNode));
...
software:
        g_software_random_next = g_software_random_next * 25214903917 + 11;
        ret |= ((g_software_random_next & 0xFFFFFFFF0000)>>16);
        g_software_random_next = g_software_random_next * 25214903917 + 11;
        ret |= ((g_software_random_next & 0xFFFFFFFF0000)<<16);
    }
    return ret;
