...
int g_number_of_processors;
U64 g_ioapic_address=0xFFFFFFFF;
U64 g_localapic_address=0xFFFFFFFF;
int parse_apic(U8* in) {
    ...
    g_localapic_address = header->LocalAPICAddress|KERNEL_ADDR_OFFSET;
    //循环处理APIC。
    ...
    while(position-in<header->Common.Length) {
        switch(*position) {
            case 0: { //Processor Local APIC
                ...
                if(table->Flags) {
                    debug(" Processor#%d :%b\n",g_number_of_processors++,table->APICID);
                }
            ...
            case 1: { //I/O APIC
                ...
                g_ioapic_address = table->IOAPICAddr|KERNEL_ADDR_OFFSET;
                g_ioapic_address = table->IOAPICAddr;
