...
void thread_test(const class* c)
{
    ...
    //压入一个测试变量。
    main_thread->stack[main_thread->rsp]=0x1234567876543210L;
    printk("Test Var is :%x\n",main_thread->stack[main_thread->rsp]);
    main_thread->pc++;
    //假设发生了跳转！
    ...
    main_thread->stack[new_stack_begin_offset] = (U64)main_thread->code;
    main_thread->stack[new_stack_begin_offset+1] = main_thread->pc;
    main_thread->stack[new_stack_begin_offset+2] = main_thread->rsp;
    main_thread->stack[new_stack_begin_offset+3] = main_thread->rbp;
    //随便压入一个临时变量试试！
    main_thread->stack[new_stack_begin_offset+sizeof(stack_frame)/sizeof(U64)+main_thread->code->max_locals] = 1;
    //加载第二个方法。
    ...
    printk("Now executing %b.\n",main_thread->code->code[main_thread->pc]);
    //返回！
    main_thread->code = (code_attribute*)main_thread->stack[main_thread->rbp];
    main_thread->pc   = main_thread->stack[main_thread->rbp+1];
    main_thread->rsp  = main_thread->stack[main_thread->rbp+2];
    main_thread->rbp  = main_thread->stack[main_thread->rbp+3];
    printk("Now executing %b.\n",main_thread->code->code[main_thread->pc]);
    printk("Test Var is :%x\n",main_thread->stack[main_thread->rsp]);
    memfree(main_thread);
}
